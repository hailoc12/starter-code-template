...
Mỗi lần AI ra quyết định, ghi audit log theo **schema canonical duy nhất** định nghĩa ở [Chương Privacy](chapter-14-privacy.md) §11.7 (ts, user hash, input_hash, output_hash, model_version, risk_level, human_approved — không lưu output/PII gốc). Đội 002 kèm artifacts eval vào repo là cùng tư duy — quyết định phải truy ngược được.
...
- Guardrail an toàn nội dung chi tiết: xem [Chương Privacy](chapter-14-privacy.md) §11.4 (prompt injection + red-team).
- Đánh giá độ tin cậy bằng eval (golden dataset, LLM-judge, pass@k/pass^k): xem chương Đánh giá hiệu quả ([chapter-08.md](chapter-08.md)).
